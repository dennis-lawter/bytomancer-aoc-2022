    ...
    fn to_string_with_depth(&self, depth: usize) -> String {
        ...
        if depth > 0 {
            result = format!(
                ...
                format!(
                    "{} B",
                    self.get_recursive_size().to_formatted_string(&Locale::en)
                )
                .pad_to_width_with_alignment(
                    80 - 1 - (depth * 4) - stripped_name.len(),
                    Alignment::Right
                )
            )
            .on_blue()
        ...
        for (_, child_dir) in &self.dirs {
            result = format!("{}\n{}", result, child_dir.to_string_with_depth(depth + 1));
        }
        for (_, child_file) in &self.files {
            result = format!(
                "{}\n{}|---{}{}",
                result,
                "|   ".repeat(depth),
                child_file.name,
                format!("{} B", child_file.size.to_formatted_string(&Locale::en))
                    .pad_to_width_with_alignment(
                        80 - ((depth + 1) * 4) - child_file.name.len(),
                        Alignment::Right
                    )
            );
        }
