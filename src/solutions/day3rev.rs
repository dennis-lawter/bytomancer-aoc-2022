...
fn char_in_three(a: &str, b: &str, c: &str) -> char {
    let a_list = a.chars().collect::<Vec<char>>();
    let b_list = b.chars().collect::<Vec<char>>();
    let c_list = c.chars().collect::<Vec<char>>();
    for i in 0..a.len() {
        for j in 0..b.len() {
            for k in 0..c.len() {
                if a_list[i] == b_list[j] && a_list[i] == c_list[k] {
