...
impl TreeMap {
    ...
    pub fn get_visible(&mut self, x: usize, y: usize) -> bool {
        self.visible[x_and_y_to_index(x, y, self.cols)]
    }
...
fn print_tree(tree_map: &mut TreeMap) {
    for x in 0..tree_map.cols {
        for y in 0..tree_map.rows {
            let test = tree_map.get_visible(x, y);
            print!("{}", if test { "🌲" } else { "　" });
        }
        println!();
    }
...
fn print_treehouse(tree_map: &mut TreeMap, house_x: usize, house_y: usize) {
    let number_list = ["０", "１", "２", "３", "４", "５", "６", "７", "８", "９"];
    for x in 0..tree_map.cols {
        for y in 0..tree_map.rows {
            let num = tree_map.get_height(x, y) as usize;
...
pub fn d8s2vis(submit: bool) {
    let mut tree_map = input();

    let mut best_score = 0u64;
    let mut best_pos = (0, 0);
    let mut best_visible: Vec<bool> = Vec::new();

    tree_map.visible = vec![false; tree_map.rows * tree_map.cols];
    ...
    for x in 1..(tree_map.cols - 1) {
        for y in 1..(tree_map.rows - 1) {
            ...
            if score > best_score {
                best_score = score;
                best_pos = (x, y);
                best_visible = tree_map.visible.clone();
            }

            tree_map.visible = vec![false; tree_map.rows * tree_map.cols];
        }
    }
    tree_map.visible = best_visible;

    print_treehouse(&mut tree_map, best_pos.0, best_pos.1);
